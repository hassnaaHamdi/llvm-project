...
void LoopVectorizationPlanner::plan(
  ...
  if (CM.EpilogueLoweringStatus == CM_EpilogueAllowed && EpilogueTailFoldingCM) {
    ...
    for_each(EnabledCMs, [&](auto *CurrentCM) {
      if (CurrentCM->blockNeedsPredicationForAnyReason(OrigLoop->getHeader())) {
        LLVM_DEBUG(dbgs() << "LV: Invalidate all interleaved groups due to "
                          << "fold-tail by masking which requires masked-interleaved support.\n");
  ...
  if (UserVF) {
    if (!ElementCount::isKnownLE(UserVF, MaxUserVF)) {
      ...
      if (EpilogueUserVF.isVector() &&
        ...
        for_each(EnabledCMs, [&](auto *CurrentCM) {
          CurrentCM->collectNonVectorizedAndSetWideningDecisions(EpilogueUserVF);
        });
